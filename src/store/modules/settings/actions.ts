import { ActionContext } from 'vuex';
import axios from 'axios';
import { InterfaceSettingsState, InterfaceCountry } from './types';

export default {
  fetchCountries(store: ActionContext<InterfaceSettingsState, any>): void {
    axios({
      url: 'https://restcountries.eu/rest/v2/all',
    }).then((response) => {
      const counties = response && response.data;
      store.commit('setCountries', counties);
    }, (error) => {
      console.log(error);
      store.commit('settingError');
    });
  },
  setSelectedCountry(store: ActionContext<InterfaceSettingsState, any>,
    country: InterfaceCountry): void {
    store.commit('setSelectedCountry', country);
  },
};
